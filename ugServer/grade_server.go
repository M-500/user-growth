...
func (s *UgGradeServer) UserGradeChange(ctx context.Context, in *pb.UserGradeChangeRequest) (*pb.UserGradeChangeReply, error) {
	...
	if gradeUser.GradeId != int(grade.Id) { // 等级更新
		newData.GradeId = int(grade.Id)
		expired := common.Now().AddDate(10, 0, 0)
		if grade.Expired > 0 {
			expired = common.Now().Add(time.Hour * 24 * time.Duration(grade.Expired))
		}
